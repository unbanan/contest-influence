<script lang="ts">
    ...
    function initGrid(two: Two, field: Group, grid: GridCell[][], R: number, r: number, sides: number, text_size: number, fld: Field[]) {
        ...
        canvasWidth = Math.max(container.clientWidth, bounds.width + padding * 2);
        canvasHeight = Math.max(container.clientHeight, bounds.height + padding * 2);

        two.width = canvasWidth;
        two.height = canvasHeight;
        two.update();
    }
    ...
    function calc_radius(container: HTMLElement) {
        let le = 0, ri = Math.max(container.clientHeight, container.clientWidth);
        const safetyMargin = 20;
        while (le + 0.001 < ri) {
            let mid = (le + ri) / 2;
            const radius = Math.sqrt(mid ** 2 - (mid / 2) ** 2);
            const y = padding + radius + h_n * radius + h_n * padding / 2 + padding + mid * 2 + safetyMargin;
            const x = padding + mid + w_n * 3 * mid + 2 * padding * w_n + padding * 2 + mid * 2 + safetyMargin;
